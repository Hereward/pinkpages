    ...
    <span><a href="/main.php?do=Content&action=contactUs">Advertise with us</a></span>    <p>Find businesses and organisations</p>
		<div class="searchbox-index-left-key">
		  ...
		  <form id="test" action="main.php" name="Homepage" method="get" onsubmit="return check();">
           ...
           <td><p class="inputbox"><input type="text" name="Search2" id="Search2" value="{if $smarty.get.p eq 's'}All Sydney{elseif $smarty.get.p eq 'n'}Newcastle Region{elseif $smarty.get.p eq 'c'}Canberra{elseif $smarty.get.p eq 'm'}Melbourne{else}All States{/if}" /></p>
           </td>
